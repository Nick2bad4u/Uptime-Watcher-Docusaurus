...
<Mermaid value={`sequenceDiagram
    ...
    Sched->>Mgr: handleScheduledCheck(siteId, monitorId)
    ...
    AppSvc->>WinSvc: sendToRenderer(channel, payload)

    WinSvc->>Renderer: ipcRenderer.emit(channel, payload)
    Renderer->>Stores: apply status update & append history
