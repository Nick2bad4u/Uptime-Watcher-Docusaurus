...
## Type-Checking Project Map

Type-checking scripts run multiple `tsc --build` projects in `config/testing`, ensuring each layer (frontend, Electron, shared, Playwright, Storybook) compiles with its own `tsconfig`. The diagram shows how meta-scripts chain them, and which caches/assets are produced.
